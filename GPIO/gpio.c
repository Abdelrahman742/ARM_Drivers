...
void GPIO_SW3_Init(void)
{
    ...
    GPIO_PORTB_PCTL_REG  &= 0x00;                             /* Clear PMCx bits for PB2 to use it as GPIO pins */
    GPIO_PORTB_DIR_REG   &= ~(1<<2) ;                         /* Configure PF0 & PF4 as input pins */
    GPIO_PORTB_AFSEL_REG &= 0x00;                             /* Disable alternative function on PB2 */
...
{
    GPIO_PORTB_IS_REG    &= ~(1<<2);      /* PA0 detect edges */
    GPIO_PORTB_IBE_REG   &= ~(1<<2);      /* PA0 will detect a certain edge */
    GPIO_PORTB_IEV_REG   &= ~(1<<2);      /* PA0 will detect a falling edge */
    GPIO_PORTB_ICR_REG   |= (1<<2);       /* Clear Trigger flag for PA0 (Interrupt Flag) */
    GPIO_PORTB_IM_REG    |= (1<<2);       /* Enable Interrupt on PA0 pin */
    /* Set GPIO PORTA priority as 6 by set Bit number 5, 6 and 7  */
    NVIC_PRI0_REG = (NVIC_PRI0_REG & GPIO_PORTB_PRIORITY_MASK) | (GPIO_PORTB_INTERRUPT_PRIORITY<<GPIO_PORTB_PRIORITY_BITS_POS);
    NVIC_EN0_REG         |= (1<<1);       /* Enable NVIC Interrupt for GPIO PORTF by set bit number 30 in EN0 Register */
}
